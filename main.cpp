        ...
        try {
            if (cmdType == "exit") {
                ...
                if (params.size() >= 1)
                    tstop = stod(params[0]);
                if (params.size() >= 2)
                    tstart = stod(params[1]);
                if (params.size() >= 3)
                    tmaxstep = stod(params[2]);
                circuit.performTransientAnalysis( tstop, tstart, tmaxstep);
            }
            else if (cmdType == ".print") {
                std::string analysisType;
                if (!(ss >> analysisType))
                    throw std::runtime_error("Invalid syntax. Analysis type expected.");
                if (analysisType == "TRAN") {
                    ...
                    if (params[0][0] == 'V' || params[0][0] == 'I')
                        throw std::runtime_error("You should insert the stop time!");
                    ...
                    if (!(ss >> sourceName >> startValue >> endValue >> increment >> variable))
                        throw std::runtime_error("Invalid syntax in getting DC analysis parameters!");
                    double startValueDouble = parseSpiceValue(startValue);
                    double endVlaueDouble = parseSpiceValue(endValue);
                    double incrementDouble = parseSpiceValue(increment);
                    ...
                    throw std::runtime_error("Analysis type not recognized!");
            }
